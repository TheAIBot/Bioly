...
namespace BiolyTests.TestObjects
{
    ...
    public class TestBlock : BiolyCompiler.BlocklyParts.FluidBlock
    {
        ...
        public TestBlock(List<FluidBlock> inputs, string output, Module associatedModule) : this(inputs.Select(input => new FluidInput(input, 1, true)).ToList(), output, associatedModule)
        {
